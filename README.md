#N1QL Indexes & Learnings

With Couchbase 4.0 the N1QL query language was introduce. This flexible query language brings SQL like query options to Couchbase Server, but what about performance?

Whenever talking about N1QL the conversion always opens up to questions about performance, what to expect in terms of performance and options to optimise the queries.

The best answer to give is most likely "it depends", but honestly that does not help much.
...
In a small application the difference of  one index on one attribute changede the execution time from +2min. to 2 seconds. No changes where made to the query itself, the only change was the index!

Note: The above query time is not for a single N1QL query but for a sequence of multiple queries in the application. 

The expected execution time for a query greatly depends on query complexity and the the system, the Couchbase Server and hardware. 

Therefore to to give a more accurate answer a test bench is needed. A well defined set of tests that can be run on different systems to revel the actual performance metrics for a given setup. In this way a measurement can be given for an actual system and query.

In this way there would be no need to claim that N1QL is fast, it could be tested on an actual system, your own setup! 
...
We are only interested in the actual execution time for the N1QL query, independant of any platform specific delays as: network delays, bootstrap time, SDK performance,  setup/cleanup times etc.   

In other words, for this particular performance test, we are completely ignorant about anything else but the 'query execution time' in the two scenarios!
...
The above metrics contains `executionTime` and this value represents the execution time on Couchbase Server, independent of network latency, platform code execution time etc. exactly what we need! 

Before running any queries we need some data to run the queries against, test data. 
...
How we create the test data is not at all important to our test nor is the time it takes to create it important. What is important is the shape of the data as it should reflect real data, best possible. Other then that we have high level of freedom on how to create it and how long time it takes.
...
Mimicking a few different documents can be achieved by changing a `type` attribute in the JSON document. Again shape is not important to Couchbase, but changing the `type` attribute we can mimic different types of documents even-though they share the same document structure.
...
The `type` attribute allows for easy search and deleting of the test documents when the test is completed, clean-up and is always given the value `perfTest`.

Loading the documents into a bucket on Couchbase Server can be done in many ways. One option would be to pre-create documents and load them into the bucket using a combination of the `cbbackup` and `cbrestore` tools. 
...
Couchbase Server will return an error if the `DROP` command is executed against an index that does not exist. This can be overcome with a check to weather or not the index exists or not:
...
###Step 2
It would probably be possible to continue using the N1QL data manipulation features to create a random data set, but also a bit more involved then creating the documents in from code.
...
The `CREATE INDEX` command is synchronous and returns when the secondary index is created and ready. That means that this command can take a while to complete, depending on the number of documents in the test. Typical times are between 3s to 2m.
...
Please note this is the exact same query as used in step 3. No changes have been made to the query it self. 

Typical execution times on my system are between 4ms to 23ms! That's a big difference! But how does data-set size affect this measure? You will have to read-on to get that answer.  
...
**Summery of the results from my system:**
...
A timeout occurs quite often when using N1QL's preview command `DELETE` on a large data-set (500.000 documents). Remember this feature is still in preview and therefore this behaviour is not to be expected when it's released, but for now it's necessary to expect this behaviour and compensate accordingly.
...
For a single line of code, that's really value for money ;)
